...
export default function MapPicker(props) {
  const {
    ...
    enabledOverride,
    clickDelay = 0,
    region: { partsByRegion, propsByRegion, partAbbreviations }
  } = props;
  ...
  const firstPart =
    initialPart ||
    (enabledParts.length > 0 ? enabledParts[enabledParts.length - 1] : "");
  ...
  const partFullName =
    (partAbbreviations && partAbbreviations[pickedPart]) || pickedPart;
  const partWords = partWordFn
    ? partWordFn(partFullName, regionName)
    : `${partFullName}, ${regionName}`;

  return (
    <div>
      <div className="clickable-state-map-county-label">{partWords}</div>
      ...
      {msg && <div className="map-picker-msg">{msg}</div>}
